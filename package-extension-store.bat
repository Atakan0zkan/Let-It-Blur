...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "try { Add-Type -AssemblyName System.IO.Compression.FileSystem; $zip = [IO.Compression.ZipFile]::OpenRead('%ZIP_PATH%'); $names = $zip.Entries.FullName | ForEach-Object { $_ -replace '\\', '/' }; $required = @('manifest.json','service-worker.js','content-script.js','popup.html','popup.css','popup.js','icons/icon128.png','icons/logo.svg','_locales/en/messages.json'); foreach ($item in $required) { if ($names -notcontains $item) { throw ('Missing from ZIP: ' + $item) } }; $blocked = @('AGENTS.md','agents.md','Agents.md','memory-bank/activeContext.md','generate-assets.js','capture-popup.js','store-assets.html'); foreach ($item in $blocked) { if ($names -contains $item) { throw ('Local-only file was packaged: ' + $item) } }; exit 0 } catch { Write-Host $_.Exception.Message -ForegroundColor Red; exit 1 } finally { if ($zip) { $zip.Dispose() } }"
if errorlevel 1 goto :fail
